...
-- ส่วนที่ 1: Number Theory & Elliptic Curve Cryptography
-- (คัดลอกส่วนนี้ก่อน)
...
EncryptStrings.SettingsDescriptor = {
    ...
    },
    {
        ...
        type = "boolean",
        default = true,
        description = "ใช้ Fully Homomorphic Encryption"
    },
    {
        name = "UseLattice",
        type = "boolean",
        default = true,
        description = "ใช้ Lattice-based Cryptography"
    },
    {
        name = "UseEllipticCurve",
        type = "boolean",
        default = true,
        description = "ใช้ Elliptic Curve Cryptography"
    }
}

function EncryptStrings:init(settings)
    self.securityLevel = settings.SecurityLevel or 3
    self.useHomomorphic = settings.UseHomomorphic ~= false
    self.useLattice = settings.UseLattice ~= false
    self.useEllipticCurve = settings.UseEllipticCurve ~= false
end
...
function EncryptStrings:CreateEncrypionService()
    ...
    -- Miller-Rabin Primality Test (ความน่าเชื่อถือสูง)
    ...
    local function create_elliptic_curve()
        ...
        local n = 0xFFFFFFFF00000000FFFFFFFFFFFFFFFFBCE6FAADA7179E84F3B9CAC2FC632551
    ...
    -- ============================================
    -- 3. LATTICE-BASED CRYPTOGRAPHY (NTRU-like)
    -- ============================================
...
local function create_fhe()
    ...
    local function ntt_poly(poly, params)
        local n = params.n
        local q = params.q
        ...
        for i = 0, n - 1 do
            ...
            for j = 0, n - 1 do
                ...
                result[i + 1] = (result[i + 1] + poly[j + 1] * w) % q
            end
        end
    ...
    local function fhe_mul(c1, c2, params)
        -- Complex multiplication using NTT
...
local function create_post_quantum_crypto()
    ...
    local function generate_dilithium_params()
        return {
            k = 4,
            l = 5,
            eta = 2,
    ...
    return {
        generate_params = generate_dilithium_params,
        sha3_like = sha3_like
    }
end
...
-- สร้าง cryptographic components
local ecc = create_elliptic_curve()
local lattice = create_lattice_crypto()
local fhe = create_fhe()
local pqc = create_post_quantum_crypto()
...
-- สร้างคีย์หลัก
local function generate_master_keys()
    -- ECC key pair
    ...
    local dilithium_params = pqc.generate_dilithium_params()
    ...
    return {
        ...
        dilithium_params = dilithium_params
    }
end

local master_keys = generate_master_keys()

-- ============================================
-- 7. MAIN ENCRYPTION ENGINE (ต่อ)
-- ============================================
...
local function advanced_encrypt(str)
    ...
    -- Layer 3: FHE Encryption
    local function fhe_encrypt_byte(byte, params)
        local plaintext = byte
        local c0 = (plaintext + fhe.generate_params().t * math.random(0, 100)) % params.q
    ...
    for i = 1, len do
        ...
        -- Apply FHE
        local fhe_enc = fhe_encrypt_byte(lwe_enc.v, master_keys.fhe_params)
...
-- ============================================
-- 8. GENERATE DECRYPTION CODE (ส่วน Runtime)
-- ============================================
...
do
    ...
    -- [1] NUMBER THEORY - Miller-Rabin & Modular Arithmetic
    local function mod_inv(a, m)
        local m0, y, x = m, 0, 1
    ...
    -- ============================================
-- 9. RUNTIME DECRYPTION (ต่อ)
-- ============================================

local function generate_decryption_code_continued()
    local code = string.format([[
    -- [3] LATTICE-BASED CRYPTOGRAPHY (LWE)
    local function lwe_decrypt(ciphertext, private_key, params)
        local u = ciphertext.u
        local v = ciphertext.v
        
        local result = 0
        for i = 1, #u do
            result = (result + u[i] * private_key[i]) %% params.q
        end
        
        result = (v - result) %% params.q
        
        -- Round to nearest plaintext
    ...
    local function DECRYPT(str, seed, metadata)
        -- Decrypt using advanced mathematics
        ...
        local lwe_private = {}
        local fhe_secret = ]] .. tostring(master_keys.fhe_params.n) .. [[
        ...
        for i = 1, #encrypted_bytes do
            ...
            local decrypted_lwe = lwe_decrypt(lwe_cipher, lwe_private, {
                q = ]] .. tostring(master_keys.lwe_params.q) .. [[,
                n = ]] .. tostring(master_keys.lwe_params.n) .. [[
            })
            ...
            local decrypted_fhe = fhe_decrypt(fhe_cipher, fhe_secret, {
...
-- ============================================
-- 10. ENCRYPTION INTERFACE
-- ============================================

local function encrypt(str)
    local encrypted_data, seed, metadata = advanced_encrypt(str)
    return encrypted_data, seed, metadata
end

-- ============================================
-- 11. FULL GENERATE CODE FUNCTION
-- ============================================

function EncryptStrings:genCode()
    local code1 = generate_decryption_code()
    local code2 = generate_decryption_code_continued()
    return code1 .. code2
end

return {
    encrypt = encrypt,
    genCode = function() return generate_decryption_code() .. generate_decryption_code_continued() end,
...
function EncryptStrings:apply(ast, pipeline)
    ...
    visitast(ast, nil, function(node, data)
        if node.kind == AstKind.StringExpression then
            ...
            -- Serialize metadata
            local metadata_str = "{" .. table.concat(
                table.map(metadata, function(m)
                    return string.format("{ecc_x1=%d,ecc_y1=%d,lwe_u={%s},lwe_v=%d,fhe_c0=%d,fhe_c1=%d}",
                        m.ecc_x1,
                        m.ecc_y1,
