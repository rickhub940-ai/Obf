...
-- EncryptStrings.lua - Custom 24-Character Alphabet Encryption
...
function EncryptStrings:CreateEncrypionService()
	...
	-- Encode Byte to 2 Alphabet Characters
	local function encode_byte(byte)
		local high = floor(byte / ALPHABET_SIZE)
...
do
	...
	local ALPHABET = ]] .. string.format("%q", ALPHABET) .. [[
	...
	function DECRYPT(str, seed)
		...
		for i = 1, len, 2 do
			...
			local originalByte = (encryptedByte + randomByte + prevVal) % 256

			output[outIndex] = char(originalByte)
			outIndex = outIndex + 1
			prevVal = originalByte
		end

		local result = concat(output)
		realStringsLocal[seed] = result
		return result
	end
end
]]
