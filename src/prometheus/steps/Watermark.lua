...
Watermark.Description = "This Step will add an Aetheris watermark to the script"
Watermark.Name = "Aetheris Watermark"

Watermark.SettingsDescriptor = {
    Content = {
        name = "Content",
        description = "The Content of the Aetheris Watermark",
        type = "string",
...
function Watermark:apply(ast)
    ...
    if string.len(self.Content) > 0 then
        local scope, variable =
            ast.globalScope:resolve(self.CustomVariable)

        local watermark =
            Ast.AssignmentVariable(
                ast.globalScope,
                variable
            )

        local functionScope =
            Scope:new(body.scope)

        functionScope:addReferenceToHigherScope(
            ast.globalScope,
            variable
        )
        ...
        local statement =
            Ast.PassSelfFunctionCallStatement(
