-- Prometheus Obfuscator - Ultra Compact Version
-- รวมทุกอย่างให้สั้นแต่หนามาก
...
function Compiler:new()
    local c = {
        ...
        upvalVars = {};
        registerUsageStack = {};
        upvalsProxyLenReturn = math.random(-2^22, 2^22);
        createClosureVars = {};
...
function Compiler:compile(ast)
    ...
    local _, gv = gScope:resolve("getfenv");
    local _, tv = gScope:resolve("table");
    local _, uv = gScope:resolve("unpack");
    local _, ev = gScope:resolve("_ENV");
    local _, nv = gScope:resolve("newproxy");
    local _, smv = gScope:resolve("setmetatable");
    local _, gmv = gScope:resolve("getmetatable");
    local _, sv = gScope:resolve("select");
    
    psc:addReferenceToHigherScope(gScope, gv, 2);
    psc:addReferenceToHigherScope(gScope, tv);
    psc:addReferenceToHigherScope(gScope, uv);
    psc:addReferenceToHigherScope(gScope, ev);
    psc:addReferenceToHigherScope(gScope, nv);
    psc:addReferenceToHigherScope(gScope, smv);
    psc:addReferenceToHigherScope(gScope, gmv);

    self.scope = Scope:new(psc);
    
    -- ใช้ชื่อสั้นมาก
    self.v = {
        env = self.scope:addVariable();
        cf = self.scope:addVariable();
        up = self.scope:addVariable();
        np = self.scope:addVariable();
        sm = self.scope:addVariable();
        gm = self.scope:addVariable();
        sl = self.scope:addVariable();
    };
    ...
    self.v.pos = self.containerFuncScope:addVariable();
    self.v.args = self.containerFuncScope:addVariable();
    self.v.cu = self.containerFuncScope:addVariable();
    self.v.dgc = self.containerFuncScope:addVariable();
    self.v.ret = self.containerFuncScope:addVariable();

    -- Upvalues
    self.v.ut = self.scope:addVariable();
    self.v.urt = self.scope:addVariable();
    self.v.auf = self.scope:addVariable();
    self.v.cuid = self.scope:addVariable();
    self.v.upf = self.scope:addVariable();
    self.v.ugf = self.scope:addVariable();
    self.v.fuf = self.scope:addVariable();

    self.createClosureVars = {};
    self.v.cvc = self.scope:addVariable();
    
    -- Closure scope
    local cs = Scope:new(self.scope);
    local cpa = cs:addVariable();
    local cua = cs:addVariable();
    local cpo = cs:addVariable();
    local cfu = cs:addVariable();
    ...
    self.getUpvalueId = function(self, scope, id)
        ...
        local expr = Ast.FunctionCallExpression(
            Ast.VariableExpression(self.scope, self.v.auf), {}
        );
    ...
    -- References
    css:addReferenceToHigherScope(self.scope, self.v.cf);
    css:addReferenceToHigherScope(cs, cpa);
    css:addReferenceToHigherScope(cs, cua, 1);
    cs:addReferenceToHigherScope(self.scope, self.v.upf);
    css:addReferenceToHigherScope(cs, cpo);
    ...
    -- Build assignments
    local ass = {{
        var = Ast.AssignmentVariable(self.scope, self.v.cf),
        val = Ast.FunctionLiteralExpression({
            Ast.VariableExpression(self.containerFuncScope, self.v.pos),
            Ast.VariableExpression(self.containerFuncScope, self.v.args),
            Ast.VariableExpression(self.containerFuncScope, self.v.cu),
            Ast.VariableExpression(self.containerFuncScope, self.v.dgc)
        }, self:emitContainerFuncBody());
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.cvc),
        val = Ast.FunctionLiteralExpression({
            Ast.VariableExpression(cs, cpa),
            Ast.VariableExpression(cs, cua),
        }, Ast.Block({
            Ast.LocalVariableDeclaration(cs, {cpo}, {
                Ast.FunctionCallExpression(
                    Ast.VariableExpression(self.scope, self.v.upf),
                    {Ast.VariableExpression(cs, cua)}
                )
            }),
            Ast.LocalVariableDeclaration(cs, {cfu}, {
                Ast.FunctionLiteralExpression({
                    Ast.VarargExpression();
                }, Ast.Block({
                    Ast.ReturnStatement{
                        Ast.FunctionCallExpression(
                            Ast.VariableExpression(self.scope, self.v.cf),
                            {
                                Ast.VariableExpression(cs, cpa),
            ...
            Ast.ReturnStatement{Ast.VariableExpression(cs, cfu)};
        }, cs)
        );
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.ut),
        val = Ast.TableConstructorExpression({}),
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.urt),
        val = Ast.TableConstructorExpression({}),
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.auf),
        val = self:createAllocUpvalFunction(),
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.cuid),
        val = Ast.NumberExpression(0),
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.upf),
        val = self:createUpvaluesProxyFunc(),
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.ugf),
        val = self:createUpvaluesGcFunc(),
    }, {
        var = Ast.AssignmentVariable(self.scope, self.v.fuf),
        val = self:createFreeUpvalueFunc(),
    }};

    local tbl = {
        Ast.VariableExpression(self.scope, self.v.cf),
        Ast.VariableExpression(self.scope, self.v.cvc),
        Ast.VariableExpression(self.scope, self.v.ut),
        Ast.VariableExpression(self.scope, self.v.urt),
        Ast.VariableExpression(self.scope, self.v.auf),
        Ast.VariableExpression(self.scope, self.v.cuid),
        Ast.VariableExpression(self.scope, self.v.upf),
        Ast.VariableExpression(self.scope, self.v.ugf),
        Ast.VariableExpression(self.scope, self.v.fuf),
    };
    
    for i, entry in pairs(self.createClosureVars) do
        table.insert(ass, entry);
        table.insert(tbl, Ast.VariableExpression(entry.var.scope, entry.var.id));
    end

    util.shuffle(ass);
    local lhs, rhs = {}, {};
    for i, v in ipairs(ass) do
        lhs[i] = v.var;
        rhs[i] = v.val;
    end
    ...
    local fn = Ast.FunctionLiteralExpression({
        Ast.VariableExpression(self.scope, self.v.env),
        Ast.VariableExpression(self.scope, self.v.up),
        Ast.VariableExpression(self.scope, self.v.np),
        Ast.VariableExpression(self.scope, self.v.sm),
        Ast.VariableExpression(self.scope, self.v.gm),
        Ast.VariableExpression(self.scope, self.v.sl),
    ...
    return Ast.TopNode(Ast.Block({
        Ast.ReturnStatement{Ast.FunctionCallExpression(fn, {
            Ast.OrExpression(
                Ast.AndExpression(
                    Ast.VariableExpression(gScope, gv),
                    Ast.FunctionCallExpression(
                        Ast.VariableExpression(gScope, gv), {}
                    )
                ),
                Ast.VariableExpression(gScope, ev)
            );
            Ast.OrExpression(
                Ast.VariableExpression(gScope, uv),
                Ast.IndexExpression(
                    Ast.VariableExpression(gScope, tv),
                    Ast.StringExpression("unpack")
                )
            );
            Ast.VariableExpression(gScope, nv);
            Ast.VariableExpression(gScope, smv);
            Ast.VariableExpression(gScope, gmv);
            Ast.VariableExpression(gScope, sv);
            Ast.TableConstructorExpression({
                Ast.TableEntry(Ast.VarargExpression());
            })
        })};
...
function Compiler:getCreateClosureVar(argCount)
    if not self.createClosureVars[argCount] then
        ...
        for i = 1, argCount do
            ...
            args[i] = Ast.VariableExpression(css, arg);
            args2[i] = Ast.TableEntry(Ast.VariableExpression(css, arg));
        end
        ...
        local val = Ast.FunctionLiteralExpression({
            ...
            Ast.LocalVariableDeclaration(cs, {cfu},{
                Ast.FunctionLiteralExpression(args,
                Ast.Block({
                    Ast.ReturnStatement{
                        Ast.FunctionCallExpression(
                            Ast.VariableExpression(self.scope, self.v.cf),
                            {
                                Ast.VariableExpression(cs, cpa),
                                Ast.TableConstructorExpression(args2),
                                Ast.VariableExpression(cs, cua),
                                Ast.VariableExpression(cs, cpo)
                            }
                        )
...
function Compiler:pos(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.pos);
    return Ast.VariableExpression(self.containerFuncScope, self.v.pos);
end

function Compiler:posAssignment(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.pos);
    return Ast.AssignmentVariable(self.containerFuncScope, self.v.pos);
end

function Compiler:args(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.args);
    return Ast.VariableExpression(self.containerFuncScope, self.v.args);
end

function Compiler:unpack(scope)
    scope:addReferenceToHigherScope(self.scope, self.v.up);
    return Ast.VariableExpression(self.scope, self.v.up);
end

function Compiler:env(scope)
    scope:addReferenceToHigherScope(self.scope, self.v.env);
    return Ast.VariableExpression(self.scope, self.v.env);
end

function Compiler:jmp(scope, to)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.pos);
    return Ast.AssignmentStatement(
        {Ast.AssignmentVariable(self.containerFuncScope, self.v.pos)},
        {to}
    );
end
...
function Compiler:setReturn(scope, val)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.ret);
    return Ast.AssignmentStatement(
        {Ast.AssignmentVariable(self.containerFuncScope, self.v.ret)},
        {val}
    );
end
...
function Compiler:getReturn(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.ret);
    return Ast.VariableExpression(self.containerFuncScope, self.v.ret);
end

function Compiler:returnAssignment(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.ret);
    return Ast.AssignmentVariable(self.containerFuncScope, self.v.ret);
end

function Compiler:setUpvalueMember(scope, idExpr, valExpr, compoundConstructor)
    scope:addReferenceToHigherScope(self.scope, self.v.ut);
...
function Compiler:createUpvaluesGcFunc()
    ...
    whileScope:addReferenceToHigherScope(self.scope, self.v.urt, 3);
    ...
    ifScope:addReferenceToHigherScope(self.scope, self.v.urt, 1);
    ifScope:addReferenceToHigherScope(self.scope, self.v.ut, 1);
    ...
    return Ast.FunctionLiteralExpression(
        ...
        Ast.Block({
            Ast.LocalVariableDeclaration(scope, {iteratorVar, valueVar},
                {Ast.NumberExpression(1), Ast.IndexExpression(
                    Ast.VariableExpression(scope, selfVar),
...
function Compiler:createFreeUpvalueFunc()
    ...
    scope:addReferenceToHigherScope(self.scope, self.v.urt, 2);
...
function Compiler:createUpvaluesProxyFunc()
    local scope = Scope:new(self.scope);
    scope:addReferenceToHigherScope(self.scope, self.v.np);
    ...
    ifScope:addReferenceToHigherScope(self.scope, self.v.np);
    ifScope:addReferenceToHigherScope(self.scope, self.v.gm);
    ifScope:addReferenceToHigherScope(self.scope, self.v.ugf);
    ifScope:addReferenceToHigherScope(scope, entriesVar);
    elseScope:addReferenceToHigherScope(self.scope, self.v.sm);
    elseScope:addReferenceToHigherScope(scope, entriesVar);
    elseScope:addReferenceToHigherScope(self.scope, self.v.ugf);

    local forScope = Scope:new(scope);
    local forArg = forScope:addVariable();
    forScope:addReferenceToHigherScope(self.scope, self.v.urt, 2);
...
function Compiler:createAllocUpvalFunction()
    local scope = Scope:new(self.scope);
    scope:addReferenceToHigherScope(self.scope, self.v.cuid, 4);
    scope:addReferenceToHigherScope(self.scope, self.v.urt, 1);

    return Ast.FunctionLiteralExpression({}, Ast.Block({
        Ast.AssignmentStatement({
            Ast.AssignmentVariable(self.scope, self.v.cuid),
        },{
            Ast.AddExpression(unpack(util.shuffle({
                Ast.VariableExpression(self.scope, self.v.cuid),
                Ast.NumberExpression(1),
...
function Compiler:emitContainerFuncBody()
    ...
    for _, block in ipairs(self.blocks) do
        ...
        -- Shuffle with dependency checking
    ...
    self.whileScope:addReferenceToHigherScope(self.containerFuncScope, self.v.ret, 1);
    self.whileScope:addReferenceToHigherScope(self.containerFuncScope, self.v.pos);
    self.containerFuncScope:addReferenceToHigherScope(self.scope, self.v.up);

    -- ใช้ table แทนการประกาศตัวแปรหลายตัว
    local declarations = { self.v.ret }
    for i, var in pairs(self.registerVars) do
        if(i ~= MAX_REGS and i <= 10) then -- เฉพาะที่จำเป็น
            table.insert(declarations, var);
        end
    end

    local stats = {
        Ast.LocalVariableDeclaration(self.containerFuncScope, 
            util.shuffle(declarations), {}
        );
        Ast.WhileStatement(whileBody, Ast.VariableExpression(self.containerFuncScope, self.v.pos));
        Ast.AssignmentStatement({
            Ast.AssignmentVariable(self.containerFuncScope, self.v.pos)
        }, {
            Ast.LenExpression(Ast.VariableExpression(self.containerFuncScope, self.v.dgc))
        }),
        Ast.ReturnStatement{
            Ast.FunctionCallExpression(
                Ast.VariableExpression(self.scope, self.v.up),
                {Ast.VariableExpression(self.containerFuncScope, self.v.ret)}
            );
        }
    }
...
function Compiler:compileTopNode(node)
    ...
    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.args);
    scope:addReferenceToHigherScope(self.scope, self.v.sl);
    scope:addReferenceToHigherScope(self.scope, self.v.up);
    self:addStatement(
...
function Compiler:compileFunction(node, funcDepth)
    ...
    self.getUpvalueId = function(self, scope, id)
        ...
        if(scopeFuncDepth == funcDepth) then
            oldActiveBlock.scope:addReferenceToHigherScope(self.scope, self.v.auf);
        ...
        elseif(scopeFuncDepth == funcDepth - 1) then
            ...
            oldActiveBlock.scope:addReferenceToHigherScope(
                self.containerFuncScope, self.v.cu
            );
    ...
    for i, arg in ipairs(node.args) do
        if(arg.kind == AstKind.VariableExpression) then
            if(self:isUpvalue(arg.scope, arg.id)) then
                scope:addReferenceToHigherScope(self.scope, self.v.auf);
    ...
    if isVarargFunction then
        scope:addReferenceToHigherScope(self.scope, self.v.cvc);
        retrieveExpression = Ast.FunctionCallExpression(
            Ast.VariableExpression(self.scope, self.v.cvc),
            {
        ...
        local varScope, var = self:getCreateClosureVar(
            #node.args + math.random(0, 5)
        );
...
function Compiler:compileBlock(block, funcDepth)
    ...
    for id, name in ipairs(block.scope.variables) do
        ...
        if self:isUpvalue(block.scope, id) then
            scope:addReferenceToHigherScope(self.scope, self.v.fuf);
            self:addStatement(
...
function Compiler:compileStatement(statement, funcDepth)
    ...
    if(statement.kind == AstKind.PassSelfFunctionCallStatement) then
        ...
        self:addStatement(
            self:setRegister(scope, tmpReg, 
                Ast.StringExpression(statement.passSelfFunctionName)
            ),
            {tmpReg}, {}, false
        );
        self:addStatement(
        ...
        if(#statement.indices > 0) then
            ...
            if statement.scope.isGlobal then
                ...
                if self.scopeFunctionDepths[statement.scope] == funcDepth then
                    ...
                    scope:addReferenceToHigherScope(self.containerFuncScope, self.v.cu);
                    self:addStatement(
                        self:setRegister(scope, tblReg,
            ...
            for i = 1, #statement.indices - 1 do
                ...
                local indexReg = self:compileExpression(
                    Ast.StringExpression(index), funcDepth, 1
                )[1];
            ...
            if primaryExpr.kind == AstKind.AssignmentVariable then
                if primaryExpr.scope.isGlobal then
                    ...
                    self:addStatement(
                        self:setRegister(scope, tmpReg,
                            Ast.StringExpression(
                                primaryExpr.scope:getVariableName(primaryExpr.id)
                            )
                        ),
                        {tmpReg}, {}, false
                    );
    ...
    if(statement.kind == AstKind.WhileStatement) then
        ...
        self:addStatement(
            self:setPos(scope, checkBlock.id),
            {self.POS_REGISTER}, {}, false
        );
    ...
    if(statement.kind == AstKind.ForStatement) then
        ...
        local tmpReg = self:allocRegister(false);
        self:addStatement(
            self:setRegister(scope, tmpReg, Ast.NumberExpression(0)),
            {tmpReg}, {}, false
        );
        ...
        tmpReg2 = self:compileExpression(
            Ast.NumberExpression(innerBlock.id), funcDepth, 1
        )[1];
        self:addStatement(
            self:setRegister(scope, self.POS_REGISTER,
                Ast.AndExpression(
        ...
        tmpReg2 = self:compileExpression(
            Ast.NumberExpression(finalBlock.id), funcDepth, 1
        )[1];
        self:addStatement(
            self:setRegister(scope, self.POS_REGISTER,
                Ast.OrExpression(
        ...
        self:addStatement(
            self:setRegister(scope, self.POS_REGISTER,
                Ast.NumberExpression(checkBlock.id)
            ),
            {self.POS_REGISTER}, {}, false
        );
    ...
    if(statement.kind == AstKind.ForInStatement) then
        ...
        self:addStatement(
            Ast.AssignmentStatement({
                ...
                Ast.FunctionCallExpression(
                    self:register(scope, exprregs[1]),
                    {
        ...
        self:addStatement(
            Ast.AssignmentStatement({
                ...
                Ast.OrExpression(
                    Ast.AndExpression(
                        self:register(scope, exprregs[3]),
                        Ast.NumberExpression(bodyBlock.id)
                    ),
                    Ast.NumberExpression(finalBlock.id)
                )
            }),
...
function Compiler:compileExpression(expression, funcDepth, numReturns)
    ...
    if(expression.kind == AstKind.PassSelfFunctionCallExpression) then
        ...
        if(returnAll or numReturns > 1) then
            ...
            self:addStatement(
                self:setRegister(scope, tmpReg,
                    Ast.StringExpression(expression.passSelfFunctionName)
                ),
                {tmpReg}, {}, false
            );
            self:addStatement(
