-- ============================================================
-- COMPILER + VM + OBFUSCATED OPERATORS - All in One
-- Original Prometheus Obfuscator by Levno_710
-- Modified to include VM and custom operators @#*$^!*?
-- ============================================================
...
-- ============================================================
-- CUSTOM OBFUSCATED OPERATORS
-- ============================================================

-- Register new AST kind for obfuscated operators
AstKind.ObfuscatedOperatorExpression = "ObfuscatedOperatorExpression";

function Ast.ObfuscatedOperatorExpression(op, lhs, rhs)
    return {
        kind = AstKind.ObfuscatedOperatorExpression,
        op = op,
        lhs = lhs,
        rhs = rhs,
    };
...
-- Operator mapping to actual Lua operations
local OBFUSCATED_OPERATORS = {
    ...
    ["$^"] = function(a, b) return (a + b) * (a - b) end,
    ["!*"] = function(a, b) return (a ^ b) % (a + 1) end,
    ["@#*$^!*?"] = function(a, b) 
...
function VM:emit(op, ...)
    local args = {...};
    table.insert(self.bytecode, { op = op, args = args });
...
function VM:setGlobal(name, value)
    self.globals[name] = value;
end

function VM:getGlobal(name)
    return self.globals[name];
end

function VM:addConstant(value)
    table.insert(self.constants, value);
    return #self.constants;
end

function VM:getConstant(idx)
...
function VM:run()
    ...
    while self.running and self.pc <= #self.bytecode do
        local ins = self.bytecode[self.pc];
        ...
        if self.debug then
            print(string.format("[VM] PC: %d, OP: %s", self.pc, op));
        end
        
        local result = self:executeInstruction(op, args);
        if result == "break" then break end
        if result == "return" then 
            if #self.callstack > 0 then
                local ret = self.callstack[#self.callstack].ret;
...
function VM:executeInstruction(op, args)
    -- Register operations
    if op == "LOADK" then
        local reg = args[1];
        local constIdx = args[2];
        self.registers[reg] = self:getConstant(constIdx);
    ...
    elseif op == "NOT" then
        local dest = args[1];
        local src = args[2];
        self.registers[dest] = not self.registers[src];
    ...
    elseif op == "AND" then
        ...
        local src1 = args[2];
        local src2 = args[3];
        local val1 = self.registers[src1];
        self.registers[dest] = val1 and self.registers[src2] or val1;
    ...
    elseif op == "OR" then
        ...
        local src2 = args[3];
        local val1 = self.registers[src1];
        self.registers[dest] = val1 or self.registers[src2];
    ...
    elseif op == "SETTABLE" then
        local tableReg = args[1];
        local keyReg = args[2];
        local valueReg = args[3];
        self.registers[tableReg][self.registers[keyReg]] = self.registers[valueReg];
        
    elseif op == "GETTABLE" then
        local dest = args[1];
        local tableReg = args[2];
        local keyReg = args[3];
        self.registers[dest] = self.registers[tableReg][self.registers[keyReg]];
        
    -- Function operations
    elseif op == "CALL" then
        local funcReg = args[1];
        local argCount = args[2];
        local retReg = args[3];
        
        local func = self.registers[funcReg];
        if type(func) == "function" then
            local callArgs = {};
            for i = 1, argCount do
                callArgs[i] = self.registers[funcReg + i];
            end
            local results = {func(unpack(callArgs))};
            for i, v in ipairs(results) do
                self.registers[retReg + i - 1] = v;
            end
            if #results > 0 then
                self.registers[retReg] = results[1];
            end
        elseif type(func) == "table" and func._type == "vm_function" then
            -- VM function call
            table.insert(self.callstack, { pc = self.pc, ret = args[4] or self.pc + 1 });
            self.pc = func._start_pc;
            return;
        end
    ...
    elseif op == "JMPIF" then
        local condReg = args[1];
        local target = args[2];
        if self.registers[condReg] then
            self.pc = target;
            return;
        end
    ...
    elseif op == "JMPIFNOT" then
        local condReg = args[1];
        local target = args[2];
        if not self.registers[condReg] then
            self.pc = target;
            return;
        end
    ...
    elseif op == "RETURN" then
        local retReg = args[1];
        local retCount = args[2];
        if retCount > 0 then
            if retCount == 1 then
                return self.registers[retReg];
            else
                local results = {};
                for i = 1, retCount do
                    results[i] = self.registers[retReg + i - 1];
                end
    ...
    -- Obfuscated operator (custom)
    elseif op == "OBFOP" then
        local dest = args[1];
        local src1 = args[2];
        local src2 = args[3];
        local opChar = args[4];
        local opFunc = OBFUSCATED_OPERATORS[opChar];
        if opFunc then
            self.registers[dest] = opFunc(self.registers[src1], self.registers[src2]);
        else
            self.registers[dest] = self.registers[src1] + self.registers[src2]; -- fallback
        end
        
    -- Debug
    elseif op == "DEBUG" then
        print("DEBUG: Register state:", self.registers);
        
    else
        error(string.format("Unknown VM instruction: %s", op));
    end
end

-- ============================================================
-- COMPILER (Modified to emit bytecode)
-- ============================================================
...
function Compiler:addConstant(value)
    if self.constMap[value] then
        return self.constMap[value];
    end
    table.insert(self.constants, value);
    local idx = #self.constants;
    self.constMap[value] = idx;
    return idx;
end
...
function Compiler:createBlock()
    ...
    local block = {
        ...
        bytecode_start = #self.bytecode + 1;
    };
...
-- ============================================================
-- COMPILER: compileTopNode (Modified for VM)
-- ============================================================
...
-- ============================================================
-- COMPILER: compileFunction (Modified for VM)
-- ============================================================
...
-- ============================================================
-- COMPILER: compileBlock (Modified for VM)
-- ============================================================
...
-- ============================================================
-- COMPILER: compileStatement (Modified for Obfuscated Operators)
-- ============================================================
...
-- ============================================================
-- COMPILER: compileExpression (Modified for Obfuscated Operators)
-- ============================================================
...
function Compiler:compileExpression(expression, funcDepth, numReturns)
    ...
    -- OBFUSCATED OPERATOR EXPRESSION (ใหม่!)
    if(expression.kind == AstKind.ObfuscatedOperatorExpression) then
        local regs = {};
        for i=1, numReturns do
            regs[i] = self:allocRegister();
            if(i == 1) then
                ...
                -- ใช้ VM instruction แทนการสร้าง AST
                -- หรือใช้ fallback เป็นฟังก์ชัน
                local op = expression.op;
                local opFunc = OBFUSCATED_OPERATORS[op];
                
                if opFunc then
                    -- สร้างฟังก์ชันชั่วคราวแล้วเรียกใช้
                    local tmpFunc = self:allocRegister(false);
                    self:addStatement(self:setRegister(scope, tmpFunc, Ast.FunctionLiteralExpression({Ast.VariableExpression(scope, scope:addVariable()), Ast.VariableExpression(scope, scope:addVariable())}, Ast.Block({
                        Ast.ReturnStatement{Ast.FunctionCallExpression(Ast.VariableExpression(scope, scope:addVariable()), {})}
                    }, Scope:new(scope)))), {tmpFunc}, {}, false);
                    -- จริงๆควรใช้ VM instruction OBFOP
                    self:addStatement(self:setRegister(scope, regs[i], Ast.FunctionCallExpression(self:register(scope, tmpFunc), {self:register(scope, lhsReg), self:register(scope, rhsReg)})), {regs[i]}, {lhsReg, rhsReg}, true);
                else
                    -- fallback
                    self:addStatement(self:setRegister(scope, regs[i], Ast.AddExpression(self:register(scope, lhsReg), self:register(scope, rhsReg))), {regs[i]}, {lhsReg, rhsReg}, true);
                end
...
return {
    ...
    VM = VM,
    Ast = Ast,
    OBFUSCATED_OPERATORS = OBFUSCATED_OPERATORS,
};
