-- ============================================
-- PROMETHEUS OBFUSCATOR - COMPLETE COMPILER
-- ============================================
...
local ObfuscatedVM = {
    ...
    execute_op = function(self, opcode, ...)
        local args = {...}
        if self._op_table[opcode] then
            return self._op_table[opcode](unpack(args))
        end
        return nil
    end,
    
    -- Run bytecode
    run = function(self, bytecode)
        self._blocks = bytecode.blocks or {}
        self._pos = bytecode.start_id or 1
        self._registers = {}
        self._return_stack = {}
        
        while self._pos do
            local block = self._blocks[self._pos]
            ...
            for _, instr in ipairs(block.instructions) do
                local op = instr.op
                local dst = instr.dst
                local src1 = instr.src1
                local src2 = instr.src2
                local const = instr.const
                
                -- Get values from registers or constants
                local val1 = src1 and self._registers[src1] or nil
                local val2 = src2 and self._registers[src2] or nil
                
                -- Execute based on opcode
                if op == "load" then
                    self._registers[dst] = const
                elseif op == "move" then
                    self._registers[dst] = self._registers[src1]
                elseif op == "getupval" then
                    self._registers[dst] = self._upvalues[const]
                elseif op == "setupval" then
                    self._upvalues[const] = self._registers[src1]
                elseif op == "getglobal" then
                    self._registers[dst] = self._env[const]
                elseif op == "setglobal" then
                    self._env[const] = self._registers[src1]
                elseif op == "call" then
                    local func = self._registers[src1]
                    local args = {}
                    for i = 1, const do
                        args[i] = self._registers[src2 + i - 1]
                    end
                    self._registers[dst] = func(unpack(args))
                elseif op == "table" then
                    local tbl = {}
                    for i = 1, const do
                        tbl[i] = self._registers[src1 + i - 1]
                    end
                    self._registers[dst] = tbl
                elseif op == "index" then
                    self._registers[dst] = self._registers[src1][self._registers[src2]]
                elseif op == "setindex" then
                    self._registers[src1][self._registers[src2]] = self._registers[dst]
                elseif op == "return" then
                    local results = {}
                    for i = 1, const do
                        results[i] = self._registers[src1 + i - 1]
                    end
                    if #self._return_stack > 0 then
                        local ret = self._return_stack[#self._return_stack]
                        self._pos = ret.pos
                        self._registers[ret.dst] = results
                        table.remove(self._return_stack)
                    end
                    return results
                else
                    -- Binary/Unary operations from symbol table
                    if dst and src1 ~= nil then
                        if src2 ~= nil then
                            self._registers[dst] = self:execute_op(op, val1, val2)
                        else
                            self._registers[dst] = self:execute_op(op, val1)
                        end
                    end
                end
            end
    ...
    -- Load bytecode from compiled source
    load_bytecode = function(self, bytecode)
        self._blocks = {}
        local block_id = bytecode.start_id
        
        -- Convert block structure
        for _, block in ipairs(bytecode.blocks) do
            local instructions = {}
            for _, stat in ipairs(block.statements) do
                local instr = self:_convert_statement(stat)
                if instr then
                    table.insert(instructions, instr)
                end
            end
            self._blocks[block.id] = {
                instructions = instructions,
                next_block = block.next_block or 0
            }
        end
        
        self._pos = bytecode.start_id
        return self
    end,
    
    -- Convert AST statement to VM instruction
    _convert_statement = function(self, stat)
        local stmt = stat.statement
        if not stmt then return nil end
        
        local instr = {
            op = _OP_TO_SYM[stmt.kind] or stmt.kind,
            dst = stat.writes and stat.writes[1] or nil,
            src1 = stat.reads and stat.reads[1] or nil,
            src2 = stat.reads and stat.reads[2] or nil,
            const = nil,
        }
        
        -- Handle special cases
        if stmt.kind == "AssignmentStatement" then
            instr.op = "move"
            instr.dst = stat.writes and stat.writes[1] or nil
            instr.src1 = stat.reads and stat.reads[1] or nil
        elseif stmt.kind == "NumberExpression" then
            instr.op = "load"
            instr.dst = stat.writes and stat.writes[1] or nil
            instr.const = stmt.value
        elseif stmt.kind == "StringExpression" then
            instr.op = "load"
            instr.dst = stat.writes and stat.writes[1] or nil
            instr.const = stmt.value
        elseif stmt.kind == "BooleanExpression" then
            instr.op = "load"
            instr.dst = stat.writes and stat.writes[1] or nil
            instr.const = stmt.value
        elseif stmt.kind == "NilExpression" then
            instr.op = "load"
            instr.dst = stat.writes and stat.writes[1] or nil
            instr.const = nil
        elseif stmt.kind == "FunctionCallExpression" then
            instr.op = "call"
            instr.dst = stat.writes and stat.writes[1] or nil
            instr.src1 = stat.reads and stat.reads[1] or nil
            instr.const = #(stat.reads or {}) - 1
        end
...
local ObfuscatedEnv = {
    ...
    new = function(self)
        local env = {}
        local protected = self._protected_keys
        
        setmetatable(env, {
            __index = function(t, k)
                local protected_key = protected[k]
                if protected_key and _SYM_TABLE[protected_key] then
                    return function(...)
                        local args = {...}
                        return _SYM_TABLE[protected_key](unpack(args))
                    end
                end
                return rawget(t, k)
            end
...
function Compiler:compile(ast)
    ...
    -- Build function node assignments
...
function Compiler:emitContainerFuncBody()
    ...
    -- Reorder statements within blocks for obfuscation
...
-- ============================================
-- MAIN: CREATE COMPILER INSTANCE AND EXPORT
-- ============================================
...
return {
    ...
    ObfuscatedVM = ObfuscatedVM,
    ObfuscatedEnv = ObfuscatedEnv,
    _SYM_TABLE = _SYM_TABLE,
    _OP_TO_SYM = _OP_TO_SYM,
    Ast = Ast,
    AstKind = AstKind,
}
